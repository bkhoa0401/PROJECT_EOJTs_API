...
class SimpleReactValidator {
    constructor(options = {}) {
      ...
      this.rules = {
        accepted             : {message: ':attribute phải được chấp nhận.',                                   rule: val => val === true, required: true},
        ...
        alpha_space          : {message: ':attribute chỉ có thể chứa các kí tự và khoảng trắng.',             rule: val => this.helpers.testRegex(val,/^[A-Z\s]*$/i)},
        alpha_num            : {message: ':attribute chỉ có thể chứa các kí tự và số.',                       rule: val => this.helpers.testRegex(val,/^[A-Z0-9]*$/i)},
        alpha_num_space      : {message: ':attribute chỉ được chứa kí tự, số và khảng trắng.',                rule: val => this.helpers.testRegex(val,/^[A-Z0-9ÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠàáâãèéêìíòóôõùúăđĩũơƯĂẠẢẤẦẨẪẬẮẰẲẴẶẸẺẼỀỀỂưăạảấầẩẫậắằẳẵặẹẻẽềềểỄỆỈỊỌỎỐỒỔỖỘỚỜỞỠỢỤỦỨỪễệỉịọỏốồổỗộớờởỡợụủứừỬỮỰỲỴÝỶỸửữựỳỵỷỹ\s]*$/i)},
        alpha_num_dot_splash : {message: ':attribute chỉ được chứa kí tự, số và khảng trắng.',                rule: val => this.helpers.testRegex(val,/^[A-Z0-9,/ÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠàáâãèéêìíòóôõùúăđĩũơƯĂẠẢẤẦẨẪẬẮẰẲẴẶẸẺẼỀỀỂưăạảấầẩẫậắằẳẵặẹẻẽềềểỄỆỈỊỌỎỐỒỔỖỘỚỜỞỠỢỤỦỨỪễệỉịọỏốồổỗộớờởỡợụủứừỬỮỰỲỴÝỶỸửữựỳỵỷỹ\s]*$/i)},
        ...
        email                : {message: ':attribute must be a valid email address.',                         rule: val => this.helpers.testRegex(val,/^[A-Z0-9.!#$%&'*+-/=?^_`{|}~]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i)},
        ...
        integer              : {message: ':attribute must be an integer.',                                    rule: val => this.helpers.testRegex(val,/^\d*$/)},
        ...
        size                 : {message: ':attribute must be :size:type.',                                    rule: (val, params) => this.helpers.size(val, params[1]) == parseFloat(params[0]), messageReplace: (message, params) => message.replace(':size', params[0]).replace(':type', this.helpers.sizeText(params[1]))},
